...
When I'm doing a code review, I start from the premise that I have no inside 
knowledge and that I am trying to follow the code as if I know nothing. Good code
should have classes and functions that give some indication of what it is trying
to do and a docstring that specifies what the code is trying to accomplish. If
it is reusable code, I am also looking for abstraction (i.e. this code works in 
multiple scenarios and not just a specific scenario.)


In a scenario with multiple computers running different OS and different
languages, a container helps to run these programs without error. In other words, 
it abstracts away differences in computers so that the communal workflow can
still run.
